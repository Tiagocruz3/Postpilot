...
function deriveCountry(location: string | null | undefined): string {
  ...
  if (/new zealand|\bnz\b/.test(lower)) return 'NZ'
  if (/(\bus\b|united states|usa|u\.s\.)/.test(lower)) return 'US'
  if (/(\buk\b|united kingdom|england|scotland|wales)/.test(lower)) return 'GB'
  if (/ireland|\beire\b/.test(lower)) return 'IE'
  if (/(canada|\bca\b)/.test(lower)) return 'CA'
  if (/australia|\baus?\b/.test(lower)) return 'AU'
  if (/singapore|\bsg\b/.test(lower)) return 'SG'
  if (/arab emirates|\buae\b|dubai|abu dhabi/.test(lower)) return 'AE'
  if (/south africa/.test(lower)) return 'ZA'
  if (/philippines|\bph\b/.test(lower)) return 'PH'
  if (/germany|deutsch/.test(lower)) return 'DE'
  if (/france|\bfr\b/.test(lower)) return 'FR'
  if (/netherlands|holland/.test(lower)) return 'NL'
